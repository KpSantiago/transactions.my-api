...
async function checkSessionIdExistence(request2, reply) {
...
async function transactionsRoutes(app2) {
  ...
  app2.get(
    ...
    async (request2, reply) => {
      ...
      const { id } = getRequestParamsSchema.parse(request2.params);
  ...
  app2.post("/", async (request2, reply) => {
    ...
    const { title, amount, type, category } = bodySchema.parse(request2.body);
...
app.addHook("preHandler", async (request2, reply) => {
...
(0, import_vitest.describe)("transactions routes", () => {
  ...
  (0, import_vitest.beforeEach)(() => {
    (0, import_node_child_process.execSync)("npx knex migrate:rollback --all");
    (0, import_node_child_process.execSync)("npx knex migrate:latest");
  });
  (0, import_vitest.it)("should be able to create a new transaction", async () => {
    await (0, import_supertest.default)(app.server).post("/transactions").send({
      title: "NEW TRANSACTION",
      amount: 700,
      type: "credit"
    }).expect(201);
  });
  (0, import_vitest.it)("should be able to list all transactions", async () => {
    const transactionsPostResponse = await (0, import_supertest.default)(app.server).post("/transactions").send({
      title: "NEW TRANSACTION",
      amount: 700,
      type: "credit"
    });
    const cookie2 = transactionsPostResponse.get("Set-Cookie");
    const getFirstTransactionId = await (0, import_supertest.default)(app.server).get("/transactions").set("Cookie", cookie2);
    (0, import_vitest.expect)(getFirstTransactionId.body).toEqual({
      total: 1,
      transactions: [
  ...
  (0, import_vitest.it)("should be able to list one transaction", async () => {
    const transactionsPostResponse = await (0, import_supertest.default)(app.server).post("/transactions").send({
      title: "NEW TRANSACTION",
      amount: 700,
      type: "credit"
    ...
    const getOneTransaction = await (0, import_supertest.default)(app.server).get(`/transactions/${transactionId}`).set("Cookie", cookie2);
    (0, import_vitest.expect)(getOneTransaction.body).toEqual({
      transactions: import_vitest.expect.objectContaining({
        title: "NEW TRANSACTION",
        amount: 700,
        type: "credit"
      })
  ...
  (0, import_vitest.it)("should be able to list transactions summary", async () => {
    const transactionsPostResponse = await (0, import_supertest.default)(app.server).post("/transactions").send({
    ...
    const cookie2 = transactionsPostResponse.get("Set-Cookie");
    await (0, import_supertest.default)(app.server).post("/transactions").set("Cookie", cookie2).send({
      title: "NEW DEBIT TRANSACTION",
      amount: 4500,
      type: "debit"
    });
    const getTransactionsSummary = await (0, import_supertest.default)(app.server).get(`/transactions/summary`).set("Cookie", cookie2);
    (0, import_vitest.expect)(getTransactionsSummary.body).toEqual({
      summary: {
        total: 2,
        amount: 4500
      }
